...
use zero_to_production::configuration::DatabaseSettings;
...
async fn spawn_app() -> TestApp {
    ...
    let mut configuration = zero_to_production::configuration::get_configuration().expect("Failed to read configuration.");
    configuration.database.database_name = format!("{}_{}", configuration.database.database_name, uuid::Uuid::new_v4());
    let connection_pool = configure_database(&configuration.database).await;

    let server = zero_to_production::startup::run(listener, connection_pool.clone()).expect("Failed to bind address");
